...
#define led 2
#define relay1 14 //relay from D5-D8
#define relay2 12
#define relay3 13
#define relay4 15
#define light_pin A0
...
#define timeMotion 3 //time for detect a motion
...
void handleMotionsensor(){
  ...
  if (server.arg("toggle") == ""){
    server.send(200, "text/plain", "toggle Command not found!");
  }else{
    statusMotion = !statusMotion;
    server.send(200, "text/plain", String(statusMotion));
  }
}
...
void loop(void){
  ...
  if (startTimer && (now - lastTrigger > (timeMotion * 1000))){
